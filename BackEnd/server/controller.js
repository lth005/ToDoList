...
router.post('/insert_usuario', (req, res, next) => {
    ...
    var query = 'insert into usuarios (nombre_usuario, correo_electronico, contraseña, id_rol, id_estado) values (?,?,md5(?),?,?);';
...
router.put('/update_contrasenia', (req, res, next) => {
    ...
    var values = [req.body.contraseña,
                req.body.id_usuario];

    var contrasenia = req.body.contraseña;
    var id = parseInt( req.body.id_usuario);

    var query = 'update usuarios set contraseña=md5(?) where id_usuario=?;';
